(ns cbot-agri-compat.main
  "Kotodama WASM entrypoint for the Cbot_agri clean-room actor (L4) — Clojure port.
  ...
  facts (`cbot_agri.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cbot_agri")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cbot_agri.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cbot_agri-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cbot_agri-compat/ready)
