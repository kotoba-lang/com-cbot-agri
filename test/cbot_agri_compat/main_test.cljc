(ns cbot-agri-compat.main-test
  "Contract + behavioral test for the cbot_agri-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "cbot_agri-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'cbot-agri-compat.main-test)]
